...
for fle in tqdm(glob('/data_1/ot-reprocessed-data/*.xml')):
    ...
    # if (predicted_event_class == 'seismic event') & \
    #         (event_class == 'seismic event'):
    #     continue
    # elif (predicted_event_class == 'seismic event') | \
    #         (event_class == 'seismic event'):
    #     input()
    #     if predicted_event_class != event_class:
    #         print(f'number seismic events: {n_seismic_events}')
    #         print(f'number blasts: {n_blasts}')
    #         try:
    #             specs = model.generate_spectrogram(st)
    #             for spec, tr in zip(specs, st.copy()):
    #                 prediction = ec2.predict_spectrogram(spec)
    #                 trace_type = type_list[prediction[0]]
