...
ets = ['seismic event', 'uncertain seismic event', 'underground blast', 
        'test pulse', 'crusher noise', 'orepass noise']
...
for et in ets:
    if (et == 'seismic event') or (et == 'uncertain seismic event'):
        status = 'accepted'
    else: 
        status = 'rejected'

    res, res = sc.events_list(event_type=et, 
